...
class VelocityGenerator:
    ...
    def start_generator(self) -> None:
        ...
        if (
            not self._speed_generator_thread
            or not self._speed_generator_thread.is_alive()
        ):
    ...
    def _get_outlier_velocity() -> float:
        ...
        return random_uniform(
            VelocityGenerator._MINIMUM_OUTLIER_VALUE,
            VelocityGenerator._MAXIMUM_OUTLIER_VALUE,
        )
    ...
    def _generate_speed_values(self) -> None:
        ...
        while self._run_thread:
            self._ramp(
                iterations=500,
                target_value=VelocityGenerator._MAXIMUM_VALUE_WITHOUT_NOISE,
            )
            self._linear(iterations=500)
            self._ramp(
                iterations=200,
                target_value=VelocityGenerator._MINIMUM_VALUE_WITHOUT_NOISE,
            )
            self._linear(iterations=200)

    def _ramp(
        self, iterations: int, target_value: float, add_noise: bool = True
    ) -> None:
        """
        Simulate a ramping velocity change.
    ...
    def _add_noise(self) -> None:
        ...
        self._noisy_value = (
            self._current_value
            + self._noise_generator.new_value() * self.noise_coefficient
        )
