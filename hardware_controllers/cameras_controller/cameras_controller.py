...
from . import CameraPosition, LightType
from .errors import (
...
class CamerasController:
    ...
    def _calculate_open_sleep_time() -> float:
        ...
        effective_sleep_time = (
            CamerasController._OPEN_CAMERAS_BASE_SLEEP_TIME + sleep_uncertainty
        )
    ...
    def _raise_on_cameras_not_ready(self) -> None:
        ...
        with self._cameras_ready_lock:
            if not self._cameras_ready:
                raise CamerasNotReadyError(
                    "Cannot trigger cameras that are not ready to be used"
                )
    ...
    def _get_pictures_available_event_by_light_type(
    ...
    def trigger(self) -> None:
        ...
        with self._global_trigger_lock:
            with self._light_type_lock:
                pictures_available = (
                    self._get_pictures_available_event_by_current_light_type()
                )

                if pictures_available.is_set():
                    raise TriggerFailedError(
                        f"Trigger failed, the previous pictures of the light type "
    ...
    def _calculate_pictures_collecting_sleep_time() -> float:
        ...
        sleep_uncertainty = random_uniform(-1, 1) * dispersion_factor
        effective_sleep_time = (
            CamerasController._COLLECTING_PICTURES_BASE_SLEEP_TIME + sleep_uncertainty
        )
    ...
    def _get_pictures(
        ...
        with self._light_type_lock:
            pictures_available = self._get_pictures_available_event_by_light_type(
                light_type
            )

            if not pictures_available.is_set():
                raise PicturesNotAvailableError(
                    f"The shutter was not triggered for the light type: {light_type.name}! "
    ...
    def _get_picture(
        self, light_type: LightType, camera_position: CameraPosition
        ...
        exposition_time = random_uniform(
            CamerasController._EXPOSITION_TIME_LOWER_LIMIT,
    ...
    @staticmethod
    def _create_picture_filepath(
        light_type: LightType, camera_position: CameraPosition
    ) -> Path:
        """
        Creates the picture filepath using the light type and camera position information's.
        ...
        filepath = (
            Path(__file__).parent.resolve().joinpath("pictures").joinpath(filename)
        )

        if not filepath.is_file():
            raise PictureNotFoundError(
                f"The picture was not found with the expected filepath {filepath}"
            )
